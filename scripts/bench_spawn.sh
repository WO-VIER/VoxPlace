...
LOCAL_BENCH_ROOT=${LOCAL_BENCH_ROOT:-"$PROJECT_ROOT/benchvps"}
REMOTE_BENCH_ROOT=${REMOTE_BENCH_ROOT:-"$REMOTE_ROOT/benchvps"}
LOCAL_DIR="$LOCAL_BENCH_ROOT/$LABEL"
REMOTE_DIR="$REMOTE_BENCH_ROOT/$LABEL"

print_usage() {
	cat <<EOF
Usage: $(basename "$0") [--help]

Benchmark complet VPS: micro-bench stockage + scénarios end-to-end.
...
Variables d'environnement:
  ...
  LOCAL_CLIENT_BIN=$LOCAL_CLIENT_BIN
  LOCAL_STORAGE_BUILD_DIR=$LOCAL_STORAGE_BUILD_DIR
  REMOTE_STORAGE_BUILD_DIR=$REMOTE_STORAGE_BUILD_DIR
  LOCAL_STORAGE_BUILD_TYPE=$LOCAL_STORAGE_BUILD_TYPE
  REMOTE_STORAGE_BUILD_TYPE=$REMOTE_STORAGE_BUILD_TYPE
  STORAGE_BENCH_BIN_NAME=$STORAGE_BENCH_BIN_NAME
  STORAGE_CHUNK_COUNT=$STORAGE_CHUNK_COUNT
  ...
  RUN_STORAGE_LOCAL=$RUN_STORAGE_LOCAL
  RUN_STORAGE_REMOTE=$RUN_STORAGE_REMOTE
  RUN_E2E=$RUN_E2E
  FORCE_E2E=$FORCE_E2E
  LABEL=$LABEL
  LOCAL_BENCH_ROOT=$LOCAL_BENCH_ROOT
  REMOTE_BENCH_ROOT=$REMOTE_BENCH_ROOT

Sorties:
  local  : \$LOCAL_BENCH_ROOT/<label>/
  remote : \$REMOTE_BENCH_ROOT/<label>/

Exemple:
  HOST=1.2.3.4 LABEL=bench_essai ./scripts/$(basename "$0")
EOF
}
...
ensure_local_client() {
	...
	cmake --build "$PROJECT_ROOT/build_release" --target VoxPlace VoxPlacePregen -j "${JOBS:-8}"
}
...
run_client() {
	...
	(
		...
		stdbuf -oL -eL env \
			...
			VOXPLACE_PROFILE_JSON="$PROFILE_JSON" \
			VOXPLACE_PROFILE_JSON_PATH="$LOCAL_DIR/${run_name}_profile.jsonl" \
...
bench_db_client = parse_client(bench_db_name)
bench_db_server = parse_server(bench_db_name)
bench_db_world_count = parse_world_count(bench_db_name)
if bench_db_client is not None and bench_db_server is not None and bench_db_world_count is not None:
    loaded = bench_db_server["loaded_window"]
    generated = bench_db_server["generated_fresh_window"]
    total_resolved = loaded + generated
    summary["e2e"][bench_db_name] = {
        "client": bench_db_client,
        "server": bench_db_server,
...
main() {
	...
	if (( should_run_e2e == 1 )); then
		...
		run_pregen "prepare_db" "bench" "BenchPrepare" "BenchPass123"
		run_scenario "bench_db" "bench" "" "BenchDB" "BenchPass123"
        
		reset_remote_pair "bench_mod"
		run_scenario "bench_modified_only" "bench_mod" "--modified-only-world" "BenchRun" "BenchPass123"
